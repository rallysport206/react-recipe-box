...
class Recipe extends React.Component {
    constructor(props) {
        ...
        this.state = {
            recipes: [
                ...
                {name: "Split Pea Soup", ingredients: ["1 pound split peas", "1 onion", "6 carrot", "4 ounce of ham"]}
            ],
            showAdd: false
        };
    ...
    showAddModal() { //show the new recipe 
    ...
    render() {
        ...
        return (
            <div className="jumbotron">
                <h1>RECIPE BOX</h1>
                <PanelGroup according id="recipes">
                    {recipes.map((recipe, index)=>(
                        <Panel eventKey={index} key={index}>
                            <Panel.Heading>
                                <Panel.Title className="title" toggle>{recipe.name}
                                </Panel.Title>
                            </Panel.Heading>
                            <Panel.Body collapsible>
                            <ListGroup>
                                {recipe.ingredients.map((ingredient,index)=>(
                                    <ListGroupItem key={index}>{ingredient}
                                    </ListGroupItem>
                                ))}
                            </ListGroup>
                            <ButtonToolbar>
                ...
                <AddRecipe onShow={this.state.showAdd} onAdd={this.addRecipe} onAddModal={this.showAddModel} />
            </div>
        );
    }
};
